...
def compute_layer_style_cost(a_S, a_G):
    ...
    a_S = tf.reshape(tf.transpose(a_S, perm=[0,3,1,2]), shape=[m, n_C, -1])
    a_G = tf.reshape(tf.transpose(a_G, perm=[0,3,1,2]), shape=[m, n_C, -1])
    ...
    #J_content =  1/(4*n_H*n_W*n_C ) *  tf.reduce_sum(tf.square(tf.subtract(a_C_unrolled, a_G_unrolled)))
    J_style_layer = 1/(np.power(2*n_C*n_H*n_W, 2)) * tf.reduce_sum(tf.square(tf.subtract(GS, GG)))
